<?php
  include("dbcon.php");
  $vendorId = 1;
  if(!isset($_GET['id']) || $_GET['id']=="") header("location: stores.php");
  else {
    $vendorId = $_GET['id'];
  }
  $vendorQ = mysqli_query($db, "SELECT * FROM vendors WHERE approved='yes' AND id='$vendorId' LIMIT 1 ");
  if(mysqli_num_rows($vendorQ)>0){
    ...
    $vendorFb = $vendorRow['fb'];
    $vendorTwitter = $vendorRow['twitter'];
    $vendorRss = $vendorRow['rss'];
  }
  else header("location: stores.php");
  ...
  <div class="container">
    <div class="breadcrumb-inner">
      <ul class="list-inline list-unstyled">
        ...
        <li class=""><a href="/stores.php">Stores</a></li>
        <li class=""><a href="/stores.php?id=<?=$vendorId?>"><?=$vendorName?></a></li>
        <?php
          if($sale=='yes') 
            echo"<li class=\"\"><a href=\"/store-detail.php?id=<?=$vendorId?>&sale=$sale\">Sale</a></li>";
          if($cat!=""){
            ...
            if(mysqli_num_rows($catNameQ)>0) {
              ...
              echo"<li class=\"\"><a href=\"/store-detail.php?id=<?=$vendorId?>&sale=$sale&cat=$cat\">$catName</a></li>";
              if($subcat!=""){
                ...
                if(mysqli_num_rows($subCatNameQ)>0){
                  ...
                  echo"<li class=\"\"><a href=\"/store-detail.php?id=<?=$vendorId?>&sale=$sale&cat=$cat&subcat=$subcat\">$subCatName</a></li>";
                }
              }
            }
          }
  ...
  <div class='container'>
    ...
    <div class='row'>
      ...
      <div class="col-md-3 sidebar"> 
        ...
        <!-- STOREBOX STARTS -->
        <div class="store-box">
        <img class="img-responsive" src="<?=$vendorLogo?>" alt="">
      <h2><a href="#"><?=$vendorName?></a></h2>
      <div class="mail-box"><a href="mailto:<?=$vendorEmail?>"><?=$vendorEmail?></a></div>
      <div class="social">
        <ul class="link">
          <?php
          if($vendorFb!=""){
            echo"
              <li class=\"fb\"><a target=\"_blank\" rel=\"nofollow\" href=\"$vendorFb\" title=\"Facebook\"></a></li>
            ";
          }
          if($vendorTwitter!=""){
            echo"
              <li class=\"tw\"><a target=\"_blank\" rel=\"nofollow\" href=\"$vendorTwitter\" title=\"Twitter\"></a></li>
            ";
          }
          if($vendorRss!=""){
            echo"
              <li class=\"rss\"><a target=\"_blank\" rel=\"nofollow\" href=\"$vendorRss\" title=\"RSS\"></a></li>
            ";
            }
          ?>
        </ul>
        ...
        <div class="head">Categories</div>
          <nav class="yamm megamenu-horizontal">
            <ul class="nav">
              <?php
                //counting sale
                $saleQ = mysqli_query($db, "SELECT id FROM products WHERE sale='yes' AND vendor = '$vendorId' ");
                $saleCount = mysqli_num_rows($saleQ);
                if($saleCount>0){
                  echo"
                    <li class=\"menu-item\"> <a href=\"store-detail.php?id=$vendorId&sale=yes\">
                    On Sale! 
                      <span style=\"
                        color: #fff;
                        background: #ee4054;
                ...
                while ($catRow = mysqli_fetch_assoc( $catQ ) ) {
                  ...
                  if(mysqli_num_rows($subCatQ)>0 ) {
                    ...
                    while ($subCatRow = mysqli_fetch_assoc($subCatQ) ) {
                              ...
                              echo("
                                <li>
                                <a href=\"store-detail.php?id=$vendorId&cat="
                                . $catId .
                                "&subcat="
                                . $subCatRow['id'] .
                                "\">"
              ...
              <?php
                ...
                while ($catRow = mysqli_fetch_assoc( $catQ ) ) {
                  ...
                  if(mysqli_num_rows($subCatQ) == 0 ) {
                    echo("
                      <li class=\"menu-item\"> <a href=\"/store-detail.php?id=$vendorId&cat=". $catRow['id'] ."\">". $catRow['name'] ."</a> </li>
                      <!-- /.menu-item -->
                      ");
                  }
                }
        ...
        <div class="sidebar-module-container">
          <div class="sidebar-filter">
            ...
            <div class="sidebar-widget wow fadeInUp animated" style="visibility: visible; animation-name: fadeInUp;">
              ...
              <form action="/store-detail.php" method="GET">
                    ...
                    <div class="dropdown dropdown-small dropdown-med dropdown-white inline">
                      ...
                      <ul role="menu" class="dropdown-menu">
                        ...
                        <?php if($order==1) echo("class = \"active\""); ?>
                          role="presentation">
                          <a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&perpage=<?=$perPage?>&page=<?=$page?>&minmax=<?=$minmax?>&order=1
                          ">Sort by latest</a>
                        </li>
                        <li 
                        <?php if($order==2) echo("class = \"active\""); ?>
                          role="presentation">
                          <a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&perpage=<?=$perPage?>&page=<?=$page?>&minmax=<?=$minmax?>&order=2
                        ...
                        <?php if($order==3) echo("class = \"active\""); ?>
                          role="presentation">
                          <a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&perpage=<?=$perPage?>&page=<?=$page?>&minmax=<?=$minmax?>&order=3
                        ...
                        <?php if($order==4) echo("class = \"active\""); ?>
                          role="presentation">
                          <a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&perpage=<?=$perPage?>&page=<?=$page?>&minmax=<?=$minmax?>&order=4
                    ...
                    <div class="dropdown dropdown-small dropdown-med dropdown-white inline">
                      ...
                      <ul role="menu" class="dropdown-menu">
                        <li role="presentation"><a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&order=<?=$order?>&page=<?=$page?>&perpage=15&minmax=<?=$minmax?>
                          ">15</a></li>
                        <li role="presentation"><a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&order=<?=$order?>&page=<?=$page?>&perpage=30&minmax=<?=$minmax?>
                          ">30</a></li>
                        <li role="presentation"><a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&order=<?=$order?>&page=<?=$page?>&perpage=60&minmax=<?=$minmax?>
                          ">60</a></li>
                        <li role="presentation"><a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&order=<?=$order?>&page=<?=$page?>&perpage=90&minmax=<?=$minmax?>
                          ">90</a></li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <?php
                    ...
                    if($pageStarts>1){
                      echo"
                        <li class=\"prev\"><a href=\"
                          /store-detail.php?id=<?=$vendorId?>&cat=$cat&subcat=$subcat&sale=$sale&order=$order&page=". (((int)($page))-1) ."&perpage=$perPage&minmax=$minmax
                          \">
                    ...
                    for ($navPage=$pageStarts; $navPage<($pageStarts+$navRange) ; $navPage++) { 
                      ...
                      echo"<li $activePage><a href=\"
                          /store-detail.php?id=<?=$vendorId?>&cat=$cat&subcat=$subcat&sale=$sale&order=$order&page=$navPage&perpage=$perPage&minmax=$minmax
                          \">$navPage</a></li>";
                    }
                  ?>
                  <li class="next">
                    <a href="
                          /store-detail.php?id=<?=$vendorId?>&cat=<?=$cat?>&subcat=<?=$subcat?>&sale=<?=$sale?>&order=<?=$order?>&page=<?=$page+1?>&perpage=<?=$perPage?>&minmax=<?=$minmax?>
                          ">
